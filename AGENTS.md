...
**Sigil** is a plugin for AI coding agents that provides a compressed memory format for encoding behavioral rules, project context, references, and user info into token-efficient symbolic notation. It achieves up to 50× compression at 100% decode accuracy (validated across 14 rounds, 37 subagent tests).
